...
const CalendarWrapper = styled(Box)(({ theme }) => {
  ...
  // calender events store access
  const store = useSelector(state => state.CalendarSlice)

  // mapping the store to get the colorCode from it
  useEffect(() => {
    store?.events?.data?.map(hex => {
      setColorHexCode(hex?.importantDateCategory?.colorCode)
    })
  }, [store?.events?.data])
      ...
      '& .fc-event': {
        ...
        '&:not(.fc-list-event)': {
          [`&.bg-${colorHexCode}`]: {
            borderColor: 'transparent',
            color: theme.palette.primary.main,
            backgroundColor: hexToRGBA(colorHexCode || '#ffff', 0.12),
        ...
        [`&.bg-${colorHexCode}`]: {
          ...
          '&:hover td': {
            backgroundColor: hexToRGBA(colorHexCode, 1)
          }
        },
