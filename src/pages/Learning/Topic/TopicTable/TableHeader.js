...
const TableHeader = props => {
  ...
  const topicData = useSelector(state => state.LearningSlice.getAllTopics)
  const AllSubject = useSelector(state => state.LearningSlice.getAllSubjectdropdown)

  // console.log('topic', topicData)
  // console.log('subject', AllSubject)

  useEffect(() => {
    if (topicData?.length > 0 || topicData?.length == 0) {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      router.replace({
        pathname: '/Learning/Topic/AddTopic'

        // query: { id: values.SubjectId }
      })
    }
  })
  ...
  return (
    <Box sx={{ p: 5, pb: 3, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end' }}>
      ...
      >
        ...
        {/* <Link href='/Learning/Topic/AddTopic' style={{ textDecoration: 'none' }}> */}
        <Button
          sx={{ mb: 2 }}
          onClick={() => {
            formik.handleSubmit()
