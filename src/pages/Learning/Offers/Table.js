...
import { deleteCoupon, DeleteSubscription, GetAllCoupon, GetAllSubscription } from 'src/store/slices/LearningSlice'
...
const RowOptions = ({ id, setView }) => {
  ...
  // ** function to handle delete offers
  const handleDelete = () => {
      ...
      >
        <MenuItem
          ...
          onClick={() => {
            ...
            router.replace({
              pathname: '/Learning/Offers/ViewOffers'
        ...
        <MenuItem
          onClick={() => {
            localStorage.setItem('id', id)
            router.replace({
              pathname: '/Learning/Offers/EditOffers'
              // query: { id: id }
            })
          }}
          sx={{ '& svg': { mr: 2 } }}
...
const UserList = ({ store }) => {
  ...
  function filterColumns(store, value) {
    ...
    return store.filter(column => {
      return (column?.CouponTitle ?? '').toLowerCase().includes(value)
    })
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        return (
          <Box>
            <Typography noWrap sx={{ color: 'text.secondary', textTransform: 'capitalize' }}>
              {row.DescountType == 'Fix' ? `₹ ${row.DescountValue}` : `${row.DescountValue} %`}
            </Typography>
          </Box>
        )
      }
    ...
    },
    {
      ...
      field: 'actions',
      headerName: 'Actions',
      renderCell: ({ row }) => <RowOptions setView={setView} id={row._id} />
    }
  ]
