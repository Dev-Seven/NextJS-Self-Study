...
const UserList = ({ apiData }) => {
  ...
  const store = useSelector(state => state.LearningSlice.allSubscription)
  ...
  function filterColumns(store, value) {
    ...
    return store.filter(column => {
      return (
        ...
        (column?.SubscriptionDetail ?? '').toLowerCase().includes(value)
      )
    })
  }
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          {isLoading === true ? (
            ...
            <Card>
              <CardHeader title='Subscription' sx={{ pb: 4, '& .MuiCardHeader-title': { letterSpacing: '.15px' } }} />
              <CardContent>
