...
import { deleteServiceTax, getAllServiceTax } from 'src/store/slices/MasterSlice'
...
const RowOptions = ({ id }) => {
  ...
  const handleDelete = () => {
    dispatch(deleteServiceTax(id)).then(res => {
      if (res?.payload?.status === 200) {
        Toast({ response: res, remove: true })
        dispatch(getAllServiceTax())
      } else {
  ...
  return (
    <>
      ...
      <div>
        <DeleteModal name='Tax' open={open} handleClose={handleClose} handleDelete={handleDelete} />
      </div>
    </>
  )
}
...
const UserList = () => {
  ...
  const store = useSelector(state => state.MasterSlice.getAllServiceTax)
  ...
  function filterColumns(store, value) {
    ...
    return store.filter(column => {
      return (column?.lable ?? '').toLowerCase().includes(value)
    })
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        return (
          <Box>
            <Typography sx={{ color: 'text.secondary' }}>{row.percentage}</Typography>
          </Box>
        )
      }
    },
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          {isLoading === true ? (
            ...
            <Card>
              <CardHeader title='Tax' sx={{ pb: 4, '& .MuiCardHeader-title': { letterSpacing: '.15px' } }} />
              <CardContent>
