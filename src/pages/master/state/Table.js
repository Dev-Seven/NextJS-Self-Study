...
import { getAllState } from 'src/store/slices/MasterSlice'
      ...
      >
        <MenuItem
          ...
          onClick={() => {
            ...
            router.replace({
              pathname: '/master/state/ViewState'
            })
          }}
        >
        ...
        <MenuItem
          onClick={() => {
            localStorage.setItem('id', id)
            router.replace({
              pathname: '/master/state/EditState'
            })
          }}
          sx={{ '& svg': { mr: 2 } }}
        >
...
const UserList = () => {
  ...
  const store = useSelector(state => state.MasterSlice.getAllState)
  ...
  function filterColumns(store, value) {
    ...
    return store.filter(column => {
      return (column?.stateName ?? '').toLowerCase().includes(value)
    })
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: 'Tax',
      renderCell: ({ row }) => {
        if (row.taxLable && Array.isArray(row.taxLable)) {
          return (
            <Box>
              {row.taxLable.map((taxLabel, index) => (
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          {isLoading === true ? (
            ...
            <Card>
              <CardHeader title='State' sx={{ pb: 4, '& .MuiCardHeader-title': { letterSpacing: '.15px' } }} />
              <CardContent>
